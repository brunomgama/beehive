...
export default function DesktopHeader({ activeIndex, onActiveChange }: DesktopHeaderProps) {
  ...
  const navItems = [
    { id: 2, label: "Dashboard", icon: Home, path: "/" },
    { id: 3, label: "Accounts", icon: Wallet, path: "/accounts" },
    { id: 5, label: "Movements", icon: ArrowLeftRight, path: "/movements" },
    { id: 6, label: "Planned", icon: CalendarClock, path: "/planned" },
    { id: 4, label: "Analytics", icon: BarChart3, path: "/analytics" },
    { id: 1, label: "Notes", icon: FileText, path: "/notes" },
    { id: 0, label: "Settings", icon: Settings, path: "/settings" },
  ]
      ...
      <div className="mx-auto px-8 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-bold cursor-pointer" onClick={() => handleNavigation(navItems[0])}>
              BeeHive
            </h1>
          </div>
