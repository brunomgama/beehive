...
import { BankAccount, bankAccountApi, AccountType } from "@/lib/api/bank/accounts-api"
...
interface AccountEditDesktopProps {
  accountId: number | null
}

export default function AccountEditDesktop({ accountId }: AccountEditDesktopProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await bankAccountApi.update(accountId, updateData)
      router.push('/accounts')
    } catch (error) {
        ...
        <div className="sticky top-0 z-50 bg-gradient-to-br from-muted/30 to-background backdrop-blur-xl border-b p-8">
          <div className="flex items-center gap-4">
            <Button
              ...
              variant="ghost"
              className="h-12 w-12 p-0 rounded-full hover:bg-white/20"
            >
              <ArrowLeft size={24} />
            </Button>
            <div>
              <h1 className="text-4xl font-bold text-foreground mb-2">Edit Account</h1>
            ...
            <div className="rounded-3xl backdrop-blur-xl border bg-white/30 p-8 shadow-xl">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="priority" type="number" value={formData.priority} onChange={(e) => setFormData({ ...formData, priority: parseInt(e.target.value) })} 
