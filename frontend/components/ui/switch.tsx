...
export const Switch = ({ children, name = "default", size = "medium", style }: SwitchProps) => {
  ...
  return (
    <SwitchContext.Provider value={{ value, setValue }}>
      <div
        className={clsx("flex w-full p-1",size === "small" && "h-8 rounded-md",size === "medium" && "h-10 rounded-md",
          size === "large" && "h-12 rounded-lg")} style={style}>
    ...
    >
      ...
      <span
        className={twMerge(clsx(
          ...
          checked && value === "source" && "bg-green-100 text-green-800 shadow-md rounded-xl",
          checked && value === "output" && "bg-red-100 text-red-800 shadow-md rounded-xl",
          !checked && "text-gray-900 hover:text-gray-1000 fill-gray-900 hover:fill-gray-1000",
