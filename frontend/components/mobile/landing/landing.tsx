...
/**
 * Landing Mobile Component
 * Financial overview for mobile devices
 */
export default function LandingMobile() {
  const { theme } = useTheme()
  const {stats, loading, error, chartConfig, themeColors, isRefreshing, handleRefresh} = useLanding()
    ...
    <div className="min-h-screen bg-gradient-to-b from-muted to-background pb-32">
      ...
      <div className="px-6 mb-6">
        <div className={`p-6 shadow-lg ${getCardStyle(theme)} relative`}>
          ...
          <p className="text-4xl font-bold text-white mb-1"> {formatBalance(stats.balance)} </p>
        ...
        <div className="bg-card rounded-3xl p-5 shadow-sm border">
          ...
          <ChartContainer config={chartConfig} className="h-[10rem] w-full">
            <AreaChart data={stats.balanceTrend} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
              ...
              {/* Actual Balance Area */}
              <Area type="monotone" dataKey="actual" stroke={themeColors.primary} 
              strokeWidth={2.5} fill="url(#actualGradient)" connectNulls={false}/>
              
              {/* Projected Balance Area */}
              <Area type="monotone" dataKey="projected" stroke={themeColors.primary} 
