import { Button } from "@/components/ui/button";
import { LiquidGlassCard } from "@/components/ui/liquid-glass";
import { PlannedMovement } from "@/lib/api/bank-api";
import { useRouter } from "next/navigation";

export function PlannedMovementsDashboardTable({ plannedMovements }: { plannedMovements: PlannedMovement[] }) {
    const router = useRouter()

    const handleRedirect = (path: string) => {
        router.push(path);
    };

    return (
        <div className="mt-6">
          <LiquidGlassCard>
            {plannedMovements.length > 0 ? (
              <ul className="space-y-2">
                {plannedMovements.map((movement) => (
                  <li key={movement.id} className="flex items-center justify-between text-white text-sm">
                    <div className="flex items-center">
                      <div className="w-8 h-8 flex items-center justify-center rounded-full bg-white/10 mr-3">
                        <span className="text-lg">{movement.category[0]?.toUpperCase()}</span>
                      </div>
                      <div>
                        <p className="font-medium">{movement.description}</p>
                        <p className="text-xs text-gray-400">
                          {new Date(movement.nextExecution).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                    <div className={`font-bold ${movement.type === 'INCOME' ? 'text-green-300' : 'text-red-300'}`}>
                      {movement.type === 'INCOME' ? '+' : '-'} {movement.amount} €
                    </div>
                  </li>
                ))}
              </ul>
            ) : (
              <p className="text-white text-sm">No planned movements.</p>
            )}
            {plannedMovements.length === 5 && (
              <div className="text-center mt-4">
                <Button variant={"ghost"} onClick={() => handleRedirect('/bank/planned-movements')} className="text-sm">
                  See All
                </Button>
              </div>
            )}
          </LiquidGlassCard>
        </div>
    )
}