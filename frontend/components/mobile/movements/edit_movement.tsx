...
export function EditMovement({accounts, loading, error, formData, onChange, onSubmit, onBack, onDelete}: EditMovementProps) {
    ...
    <div className="flex flex-col min-h-screen bg-background">
      <header className="flex items-center justify-between px-4 pt-4 pb-3 mb-6">
        ...
        <div className="flex gap-2">
          {onDelete && (
            <Button type="button" variant="outline" onClick={onDelete} className="w-12 h-12 rounded-2xl bg-white shadow-sm flex items-center justify-center">
      ...
      <form onSubmit={onSubmit} className="flex flex-col flex-1 px-4 pb-10 space-y-4">
        ...
        <div className="rounded-3xl bg-card shadow-sm px-4 py-3 min-h-[5rem] flex flex-col justify-between">
          <div className="flex items-center justify-between text-xs text-normal-blue">
            <span>Amount</span>
            <span>
              {selectedAccount ? `From ${selectedAccount.accountName}` : "Select account"}
            </span>
          </div>

          <div className="mt-2 relative">
            <span className={`absolute left-0 top-1/2 -translate-y-1/2 text-2xl font-sf-display ${formData.amount === 0 ? "text-[#737373]" : "text-color"}`}>
              €
            </span>
            <Input id="amount" name="amount" type="number" inputMode="decimal" 
            placeholder="0.00" value={formData.amount === 0 ? "" : formData.amount} 
        ...
        <div className="rounded-2xl bg-card shadow-sm px-4 py-3 flex text-xs min-h-[5rem] w-full">
          <div className="flex flex-col space-y-1 flex-1">
            <span className="text-normal-blue mb-3">Account</span>

            <select id="accountId" name="accountId" value={formData.accountId} onChange={onChange}
              className="bg-transparent text-text-color text-sm focus:outline-none w-full pr-2" >
        ...
        <div className="rounded-2xl bg-card shadow-sm px-4 py-3 flex text-xs min-h-[5rem] w-full">
          <div className="flex flex-col space-y-1 flex-1">
            <span className="text-normal-blue mb-3">Type</span>
        ...
        <div className="rounded-2xl bg-card shadow-sm px-4 py-3 flex text-xs min-h-[5rem] w-full">
          <div className="flex flex-col space-y-1 flex-1">
            <span className="text-normal-blue mb-3">Status</span>
        ...
        <div className="rounded-2xl bg-card shadow-sm px-4 py-3 min-h-[5rem] flex flex-col justify-between">
          ...
          <Input id="date" borderless name="date" type="date" value={formData.date} onChange={onChange}
            className="w-full bg-transparent text-sm text-text-color focus:outline-none"/>
        </div>
